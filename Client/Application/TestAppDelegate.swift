...
class TestAppDelegate: AppDelegate {
    override func getProfile(_ application: UIApplication) -> Profile {
        ...
        if launchArguments.contains(LaunchArguments.ClearProfile) {
            ...
            os_log(.debug, log: Log.tests, "Deleting all files in 'Documents' directory to clear the profile")
    ...
    func resetApplication() {
        os_log(.debug, log: Log.tests, "Wiping everything for a clean start.")
        ...
        for content in docContents {
            do {
                ...
                os_log(.debug, log: Log.tests, "Couldn't delete some document contents.")
            }
        }
    }
